...
// ─── ONLY CHANGED FUNCTION: normaliseTransactions ────────────────────────────
// Handles: bare array, wrapped { data:[] }, wrapped { transactions:[] },
// varied field names (amount/value/sum, name/title/description, date/created_at)
...
// 👇 ALREADY EXISTING
function TopBar({ dark, setDark, setMobileOpen, theme, search, setSearch, handleSearch, notificationsOn }) {
  return (
    <div className={cx("sticky top-0 z-20 border-b backdrop-blur-2xl", theme.topbar)}>
      <div className="flex items-center gap-3 px-4 py-4 sm:px-6 xl:px-8">
        ...
        <ThemeToggle dark={dark} onToggle={() => setDark((v) => !v)} theme={theme} />
...
function DashboardSection({
  theme, dark, range, setRange, filterOpen, setFilterOpen, filterRef,
  filteredTransactions, prediction, totalBalance, income, expenses, savings, loading,
}) {
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className={cx("rounded-2xl border px-4 py-3", theme.mutedPanel)}>
              ...
              <div className={cx("mt-2 text-xl font-semibold", theme.text)}>
                {loading ? "—" : formatMoney(savings)}
              </div>
            </div>
            <div className={cx("rounded-2xl border px-4 py-3", theme.mutedPanel)}>
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
        <StatCard
          title="Total Balance"
          value={loading ? "Loading…" : formatMoney(totalBalance)}
          change="+8.4%"
          positive
          icon={Wallet}
          theme={theme}
        ...
        <StatCard
          title="Expenses"
          value={loading ? "Loading…" : formatMoney(Math.abs(expenses))}
          change="+3.2%"
          positive={false}
          icon={TrendingDown}
          theme={theme}
                ...
                {filterOpen && (
                  ...
                  >
                    {ranges.map((option) => (
                      <button
                        key={option}
                        onClick={() => { setRange(option); setFilterOpen(false); }}
                        className={cx(
                          "block w-full px-4 py-3 text-left text-sm transition-all duration-200",
                          range === option
                            ? dark ? "bg-violet-500/20 text-white" : "bg-violet-50 text-slate-900"
                            : dark ? "text-slate-300 hover:bg-white/5 hover:text-white" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"
                        )}
                      >
                        {option}
                      </button>
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trendData[range]} margin={{ top: 5, right: 10, left: -12, bottom: 0 }}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{ fill: dark ? "#94a3b8" : "#64748b", fontSize: 12 }} tickFormatter={(v) => `${Math.round(v / 1000)}k`} />
          ...
          <div className="overflow-hidden rounded-[26px] border border-inherit">
            ...
            <div className={cx("divide-y", theme.divider)}>
              {filteredTransactions.length === 0 ? (
                <div className={cx("px-4 py-8 text-center text-sm", theme.mutedText)}>
...
function InsightsSection({
  theme, dark, predictionInput, setPredictionInput,
  years, setYears, returnRate, setReturnRate, prediction, filteredInsights,
}) {
  return (
    <div className="space-y-6">
      <section className={cx("rounded-[32px] border p-6", theme.panel)}>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => setPredictionInput(e.target.value.replace(/[^\d]/g, ""))}
            ...
            <div>
              ...
              <input
                type="number" min="1" max="30" step="0.5" value={returnRate}
...
function SettingsSection({ theme, dark, settings, setSettings, setDark }) {
  const settingRows = [
    { key: "notifications", title: "Real-time notifications", description: "Get alerts for large transactions, savings milestones, and portfolio changes.", icon: Bell },
    { key: "autoInsights", title: "Auto AI insights", description: "Generate automatic recommendations whenever new financial activity is detected.", icon: BrainCircuit },
    { key: "safeMode", title: "Privacy safe mode", description: "Keep projections and analytics visible only inside your personal workspace.", icon: ShieldCheck },
  ];
      ...
      <section className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_0.9fr]">
                ...
                <div key={item.key} className={cx("flex items-center justify-between gap-4 rounded-[26px] border p-4", theme.mutedPanel)}>
                  ...
                  <button
                    ...
                    className={cx(
                      ...
                      enabled ? "border-cyan-400/30 bg-cyan-400/20" : dark ? "border-white/10 bg-white/10" : "border-slate-300 bg-slate-200"
                    )}
                  >
                    <span className={cx("absolute top-1 flex h-6 w-6 items-center justify-center rounded-full bg-white text-slate-900 transition-all duration-300", enabled ? "left-9" : "left-1")}>
        ...
        <Panel theme={theme}>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <button onClick={() => setDark(true)} className={cx("rounded-[26px] border p-5 text-left transition-all duration-300", dark ? "border-violet-400/30 bg-violet-500/10" : theme.mutedPanel)}>
            ...
            <button onClick={() => setDark(false)} className={cx("rounded-[26px] border p-5 text-left transition-all duration-300", !dark ? "border-cyan-400/30 bg-cyan-400/10" : theme.mutedPanel)}>
...
export default function App() {
  const [dark, setDark] = useState(() => {
    try { return localStorage.getItem("finsight-theme") !== "light"; } catch { return true; }
  });
  ...
  const [settings, setSettings] = useState({ notifications: true, autoInsights: true, safeMode: true });
  ...
  // ── FIXED: useEffect with hardened normaliser via normaliseTransactions() ──
  useEffect(() => {
    let cancelled = false;
    ...
    fetch("http://localhost:5000/transactions")
      ...
      .then((data) => {
        ...
        const formatted = list.map((t) => {
          const amount = Number(
            t.amount ?? t.value ?? t.sum ?? 0
          );
  
          return {
            name: t.name || t.title || "Transaction",
  ...
  // ── Totals computed from live transactions state ───────────────────────────
  const totals = useMemo(() => {
    const income = transactions.filter((t) => t.positive).reduce((s, t) => s + t.amount, 0);
    const expenses = transactions.filter((t) => !t.positive).reduce((s, t) => s + t.amount, 0);
    return { income, expenses, totalBalance: income + expenses, savings: income - Math.abs(expenses) };
  }, [transactions]);

  // ── Smart insights derived from live totals ───────────────────────────────
  const smartInsights = useMemo(() => {
    const { income, expenses, savings } = totals;
    const absExpenses = Math.abs(expenses);
    const expenseRatio = income > 0 ? absExpenses / income : 0;
    const recentNegativeCount = transactions.slice(0, 5).filter((t) => !t.positive).length;
    const negativeHeavy = recentNegativeCount >= 3;
    const cards = [...BASE_INSIGHT_CARDS];

    cards[0] = expenseRatio > 0.7
      ? { ...cards[0], title: "⚠️ High expense-to-income ratio", body: `Your expenses are ${Math.round(expenseRatio * 100)}% of your income this period — above the 70% threshold. Consider cutting discretionary spend to improve savings health.` }
      : { ...cards[0], title: "Spending within safe limits", body: `Your expense ratio is ${Math.round(expenseRatio * 100)}% of income — comfortably under 70%. You have room to increase your SIP contributions this month.` };

    cards[1] = {
      ...cards[1],
      title: savings >= 0 ? "Savings are positive" : "Savings deficit detected",
      body: savings >= 0
        ? `You have saved ${new Intl.NumberFormat("en-IN").format(Math.round(savings))} this period after all expenses. Investing a portion in index funds could compound this significantly.`
        : `Your expenses exceed income by ${new Intl.NumberFormat("en-IN").format(Math.abs(Math.round(savings)))}. Review non-essential spend to restore a positive savings balance.`,
      tone: savings >= 0 ? "cyan" : "rose",
    };

    cards[2] = negativeHeavy
      ? { ...cards[2], title: "Recent transactions skew negative", body: `${recentNegativeCount} of your last 5 transactions are outflows. This spending burst may impact your month-end savings target.`, tone: "rose" }
      : { ...cards[2], title: "Healthy recent transaction mix", body: "Your recent transaction history shows a good income-to-expense balance. Keep maintaining this pattern to stay ahead of your quarterly savings milestone.", tone: "emerald" };
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem("finsight-theme", dark ? "dark" : "light"); } catch {}
  }, [dark]);
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter((t) =>
      `${t.name} ${t.category} ${t.date}`.toLowerCase().includes(searchValue)
    );
  }, [transactions, searchValue]);

  const filteredInsights = useMemo(() => {
    if (!searchValue) return smartInsights;
    return smartInsights.filter((c) =>
      `${c.title} ${c.body}`.toLowerCase().includes(searchValue)
    );
  }, [smartInsights, searchValue]);
  ...
  const prediction = useMemo(() => {
    ...
    return { monthlyInvestment, years: yearsNum, returnRate: rateNum, totalInvested, futureValue: Math.round(futureValue), wealthGain: Math.round(futureValue - totalInvested) };
  ...
  const handleSearch = (e) => {
    ...
    if (matchedNav) { setActiveNav(matchedNav.id); return; }
    if (filteredInsights.length) { setActiveNav("insights"); return; }
    if (filteredTransactions.length !== transactions.length) { setActiveNav("dashboard"); return; }
  ...
  return (
    <div className={dark ? "dark" : ""}>
      <style>{`
        ...
        body { text-align: left; font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; transition: background 240ms ease-in-out, color 240ms ease-in-out; }
        #root { border: 0; margin: 0; display: block; }
        * { box-sizing: border-box; }
      `}</style>

      <div className={cx("min-h-screen transition-colors duration-300", theme.app)}>
        <Sidebar activeNav={activeNav} setActiveNav={setActiveNav} theme={theme} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} dark={dark} />

        <div className="min-h-screen xl:pl-72">
          <TopBar dark={dark} setDark={setDark} setMobileOpen={setMobileOpen} theme={theme} search={search} setSearch={setSearch} handleSearch={handleSearch} notificationsOn={settings.notifications} />
          ...
          <main className="px-4 py-6 sm:px-6 xl:px-8">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {activeNav === "dashboard" && (
                  <DashboardSection
                    theme={theme} dark={dark} range={range} setRange={setRange}
                    filterOpen={filterOpen} setFilterOpen={setFilterOpen} filterRef={filterRef}
                    filteredTransactions={filteredTransactions} prediction={prediction}
                    totalBalance={totals.totalBalance} income={totals.income}
                    expenses={totals.expenses} savings={totals.savings} loading={loading}
                  />
                )}
                {activeNav === "analytics" && <AnalyticsSection theme={theme} dark={dark} range={range} />}
                {activeNav === "insights" && (
                  <InsightsSection
                    theme={theme} dark={dark} predictionInput={predictionInput}
                    setPredictionInput={setPredictionInput} years={years} setYears={setYears}
                    returnRate={returnRate} setReturnRate={setReturnRate}
                    prediction={prediction} filteredInsights={filteredInsights}
                  />
                )}
                {activeNav === "settings" && (
